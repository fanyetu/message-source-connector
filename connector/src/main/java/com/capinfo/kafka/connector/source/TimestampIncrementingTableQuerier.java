...
import com.capinfo.kafka.connector.source.SchemaMapping.FieldSetter;
import com.capinfo.kafka.connector.source.TimestampIncrementingCriteria.CriteriaValues;
...
public class TimestampIncrementingTableQuerier extends TableQuerier implements CriteriaValues {
    ...
    protected final String sourceColumnName;
    protected final String targetColumnName;
    ...
    public TimestampIncrementingTableQuerier(DatabaseDialect dialect, QueryMode mode, String name,
        ...
        partition = Collections.singletonMap(MessageSourceConnectorConstants.QUERY_NAME_KEY,
    ...
    public SourceRecord extractRecord() throws SQLException {
        ...
        // 组装topic
        String target = resultSet.getString(targetColumnName);
        String topic = StringUtils.isBlank(target) ? topicPrefix + resultSet.getString(sourceColumnName)
                : topicPrefix + target;
        if (StringUtils.isBlank(topic)) {
            log.warn("该数据的topic为空");
            throw new DataException("topic is null");
        }
