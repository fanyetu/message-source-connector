...
import com.capinfo.kafka.connector.dialect.DatabaseDialectProvider.JdbcUrlInfo;
 ...
 * include in your JAR file a {@code META-INF/services/com.capinfo.kafka.connector.dialect
...
public class DatabaseDialects {
    ...
    private static void loadAllDialects() {
        LOG.info("Searching for and loading all JDBC source dialects on the classpath");
...
//                        } catch (Throwable t) {
//                            LOG.info("Skipping dialect provider after error while loading", t);
//                        }
//                    }
//                } catch (Throwable t) {
//                    LOG.info("Error loading dialect providers", t);
//                }
//                return null;
//            }
//        });
        REGISTRY.put(MySqlDatabaseDialect.Provider.class.getName(), new MySqlDatabaseDialect.Provider());
        LOG.info("Registered {} source dialects", REGISTRY.size());
    }
    ...
    public static DatabaseDialect findBestFor(
        ...
        LOG.info("Finding best dialect for {}", info);
        ...
        for (DatabaseDialectProvider provider : REGISTRY.values()) {
            ...
            LOG.info("Dialect {} scored {} against {}", provider, score, info);
            if (score > bestScore) {
                bestMatch = provider;
                bestScore = score;
            }
        }
        LOG.info("Using dialect {} with score {} against {}", bestMatch, bestScore, info);
    ...
    public static DatabaseDialect create(
        ...
        LOG.info("Looking for named dialect '{}'", dialectName);
