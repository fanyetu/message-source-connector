...
import com.capinfo.kafka.connector.dialect.DatabaseDialectProvider.FixedScoreProvider;
import com.capinfo.kafka.connector.source.ColumnMapping;
import com.capinfo.kafka.connector.source.MessageSourceConnectorConfig;
import com.capinfo.kafka.connector.source.MessageSourceConnectorConfig.NumericMapping;
import com.capinfo.kafka.connector.source.MessageSourceConnectorConfig.TimestampGranularity;
import com.capinfo.kafka.connector.source.MessageSourceTaskConfig;
import com.capinfo.kafka.connector.source.TimestampIncrementingCriteria;
import com.capinfo.kafka.connector.util.*;
import com.capinfo.kafka.connector.util.ColumnDefinition.Mutability;
import com.capinfo.kafka.connector.util.ColumnDefinition.Nullability;
...
public class GenericDatabaseDialect implements DatabaseDialect {
    ...
    public static class Provider extends FixedScoreProvider {
        public Provider() {
            super(GenericDatabaseDialect.class.getSimpleName(),
                    com.capinfo.kafka.connector.dialect.DatabaseDialectProvider.AVERAGE_MATCHING_SCORE
            );
        }
    ...
    protected final com.capinfo.kafka.connector.dialect.DatabaseDialectProvider.JdbcUrlInfo jdbcUrlInfo;
    ...
    private final MessageSourceConnectorConfig.TimestampGranularity tsGranularity;
    ...
    protected GenericDatabaseDialect(
        ...
        this.jdbcUrl = config.getString(MessageSourceConnectorConfig.CONNECTION_URL_CONFIG);
        this.jdbcUrlInfo = DatabaseDialects.extractJdbcUrlInfo(jdbcUrl);
//    if (config instanceof JdbcSinkConfig) {
//      JdbcSinkConfig sinkConfig = (JdbcSinkConfig) config;
//      catalogPattern = MessageSourceTaskConfig.CATALOG_PATTERN_DEFAULT;
//      schemaPattern = MessageSourceTaskConfig.SCHEMA_PATTERN_DEFAULT;
...
//      );
//    } else {
        catalogPattern = config.getString(MessageSourceTaskConfig.CATALOG_PATTERN_CONFIG);
        schemaPattern = config.getString(MessageSourceTaskConfig.SCHEMA_PATTERN_CONFIG);
        tableTypes = new HashSet<>(config.getList(MessageSourceTaskConfig.TABLE_TYPE_CONFIG));
        quoteSqlIdentifiers = QuoteMethod.get(
                config.getString(MessageSourceConnectorConfig.QUOTE_SQL_IDENTIFIERS_CONFIG)
        );
//    }
//    if (config instanceof MessageSourceConnectorConfig) {
        mapNumerics = ((MessageSourceConnectorConfig) config).numericMapping();
        batchMaxRows = config.getInt(MessageSourceConnectorConfig.BATCH_MAX_ROWS_CONFIG);
//    } else {
...
//    }
        ...
        if (config instanceof MessageSourceConnectorConfig) {
            timeZone = ((MessageSourceConnectorConfig) config).timeZone();
...
//      timeZone = ((JdbcSinkConfig) config).timeZone;
        ...
        if (config instanceof MessageSourceConnectorConfig) {
            tsGranularity = TimestampGranularity.get((MessageSourceConnectorConfig) config);
        } else {
    ...
    public Connection getConnection() throws SQLException {
        ...
        String username = config.getString(MessageSourceConnectorConfig.CONNECTION_USER_CONFIG);
        Password dbPassword = config.getPassword(MessageSourceConnectorConfig.CONNECTION_PASSWORD_CONFIG);
    ...
    protected Properties addConnectionProperties(Properties properties) {
        ...
        config.originalsWithPrefix(MessageSourceConnectorConfig.CONNECTION_PREFIX).forEach((k, v) -> {
            if (!configKeys.contains(MessageSourceConnectorConfig.CONNECTION_PREFIX + k)) {
                properties.put(k, v);
            }
        });
        return properties;
     ...
     * the {@link MessageSourceConnectorConfig#BATCH_MAX_ROWS_CONFIG batch size} of the connector.
     ...
     * @param sqlType    the JDBC {@link Types type} as obtained from the column definition
